...
void InverseofFactorial(int point) {
 factorialNumInverse[0] = factorialNumInverse[1] = 1;
 for (int i = 2; i <= N; i++)
  factorialNumInverse[i] = (naturalNumInverse[i] * factorialNumInverse[i - 1]) % point;
}
void factorial(int point) {
 fact[0] = 1;
 for (int i = 1; i <= N; i++) {
  fact[i] = (fact[i - 1] * i) % point;
 }
}
int Binomial(int Nt, int Ri, int pi) {
    if(Ri>Nt) return 0;
    if(Ri==Nt) return 1;
 int answer = ((fact[Nt] * factorialNumInverse[Ri]) % pi * factorialNumInverse[Nt - Ri]) % pi;
 return answer;
}
